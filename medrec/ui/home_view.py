...
class HomeView(Page):
    def __init__(self, parent, controller=None):
        ...
        heading = customtkinter.CTkLabel(
            new_entry_frame, text="New Entry", font=("Roboto", 15))
        heading.pack(fill=Y)

        paragraph = customtkinter.CTkLabel(
            new_entry_frame, text="Create a new entry")
        paragraph.pack(fill=Y)

        self.new_entry_button = customtkinter.CTkButton(
            new_entry_frame, text="New Entry", command=self.new_entry)
        self.new_entry_button.pack(fill=Y)

        # a frame for the view entries dialog
        view_entries_frame = customtkinter.CTkFrame(self)
        view_entries_frame.pack(side=RIGHT, fill=BOTH,
                                expand=1, padx=20, pady=40)

        heading = customtkinter.CTkLabel(
            view_entries_frame, text="View Entries", font=("Roboto", 15))
        heading.pack(fill=Y)

        paragraph = customtkinter.CTkLabel(
            view_entries_frame, text="View your entries")
        paragraph.pack(fill=Y)

        self.view_entries_button = customtkinter.CTkButton(
            view_entries_frame, text="View Entries", command=self.view_entries)
